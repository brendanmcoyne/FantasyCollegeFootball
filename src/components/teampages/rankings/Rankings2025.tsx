...
export default function Rankings() {
    ...
    const ranking = [...teams].sort(
        (a, b) =>
            Rankings2025[selectedRanking].getValue(b) -
            Rankings2025[selectedRanking].getValue(a)
    )
    ...
    return (
        <RankingsPage>
            ...
            <RankingsCard>
                ...
                >
                    {Object.entries(Rankings2025).map(
                        ([key, ranking]) => (
                ...
                <RankingList>
                    {ranking.map((team, index) => {
                        const value = Rankings2025[selectedRanking].getValue(team)

                        const previousValue = index > 0 ? Rankings2025[selectedRanking].getValue(ranking[index - 1]) : null
                        ...
                        const rank =
                            index > 0 && value === previousValue
                                ? (() => {
                                    ...
                                    while (
                                        tiedIndex > 0 &&
                                        Rankings2025[selectedRanking].getValue(ranking[tiedIndex]) ===
                                        Rankings2025[selectedRanking].getValue(ranking[tiedIndex - 1])
                                        ) {
                                        tiedIndex--
                                    }
